...
Responde de manera natural y útil.`;
  ...
  async createSession(userId, clinicId, userRole = 'doctor') {
    try {
      ...
      console.error('[ConversationService] Error creating session:', err);
      throw err;
    }
  }
  ...
  async processUserInput(sessionId, userMessage, userId, clinicId, userRole = 'doctor') {
    try {
      ...
      const claudeResponse = await this.claudeClient.chat(messages, systemPrompt);

      // Check if Claude is calling tools
      if (claudeResponse.stop_reason === 'tool_use') {
        const toolCalls = this.claudeClient.extractToolCalls(claudeResponse.content);
        ...
        // Build tool results content for next Claude call
        ...
        const finalResponse = await this.claudeClient.chat(messages, systemPrompt);
        const finalText = this.claudeClient.extractTextResponse(finalResponse.content);
        ...
        const responseText = this.claudeClient.extractTextResponse(claudeResponse.content);
