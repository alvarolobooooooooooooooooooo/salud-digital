...
Responde de manera natural y útil.`;
  ...
  async processUserInput(sessionId, userMessage, userId, clinicId, userRole = 'doctor') {
    try {
      ...
      const claudeResponse = await this.llmClient.chat(messages, systemPrompt);

      // Check if Claude is calling tools
      if (claudeResponse.stop_reason === 'tool_use') {
        const toolCalls = this.llmClient.extractToolCalls(claudeResponse.content);
        ...
        const finalResponse = await this.llmClient.chat(messages, systemPrompt);
        const finalText = this.llmClient.extractTextResponse(finalResponse.content);
        ...
        const responseText = this.llmClient.extractTextResponse(claudeResponse.content);
