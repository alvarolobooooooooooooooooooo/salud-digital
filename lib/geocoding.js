...
// ── Fondo de la cola ──
// La cola es global al proceso y avanza a una tarea por segundo (el ToS de
// Nominatim). Sin tope, el buscador de direcciones es un amplificador: cada
// petición encolada retiene un socket abierto durante TODA la espera, así que
// N peticiones baratas se convierten en N sockets bloqueados N segundos. Con
// 300 encoladas, la última espera más de cinco minutos y para entonces el
// proceso está sujetando 300 conexiones que no hacen nada. Y como la cola la
// comparten el alta pública y el buscador de la app, llenarla deja sin
// direcciones a todo el mundo, no solo a quien abusa.
//
// Con fondo, el que llega cuando está llena recibe un "ahora no" inmediato y
// suelta la conexión. Es peor respuesta que esperar, pero es una respuesta.
const MAX_EN_COLA = parseInt(process.env.GEOCODE_MAX_QUEUE || '25', 10);
let enCola = 0;

class ColaLlena extends Error {
  constructor() {
    super('La cola de geocodificación está llena');
    this.name = 'ColaLlena';
    this.colaLlena = true;
  }
}
...
function enqueue(task) {
  if (enCola >= MAX_EN_COLA) return Promise.reject(new ColaLlena());
  enCola++;
  const run = async () => {
  ...
  const next = chain.then(run, run).finally(() => { enCola--; });
  chain = next.catch(() => {});
  return next;
}

/** Cuántas tareas hay esperando ahora mismo (para pruebas y diagnóstico). */
function tamanoDeCola() {
  return enCola;
}
...
async function backfillMissing(limit = 200) {
  ...
  for (const r of rows) {
    // Un momento de cola llena (alguien buscando direcciones en la app) no puede
    // llevarse por delante el resto del backfill: se salta esa clínica y sigue.
    try {
      await geocodeAndStore(r.id, { address: r.address, city: r.city });
    } catch (err) {
      if (err && err.colaLlena) {
        console.warn('[geocode-backfill] cola llena, se reintenta en el próximo arranque');
        return;
      }
      console.warn('[geocode-backfill] clínica', r.id, err.message);
    }
  }
}

module.exports = {
  geocodeAddress, searchAddress, geocodeAndStore, backfillMissing,
  ColaLlena, tamanoDeCola, MAX_EN_COLA,
};
