...
const tools = {
  ...
  schedule_appointment: async (params, user) => {
    try {
      ...
      if (patientResult.rows.length === 0) {
        return { success: false, error: `Paciente "${patient_name}" no encontrado` };
      }
      ...
      return {
        ...
        data: {
          ...
          date,
          time,
          message: `Cita agendada con ${patient_name} el ${date} a las ${time}.`
        }
      };
    } catch (err) {
      return {
  ...
  reschedule_appointment: async (params, user) => {
    try {
      ...
      const scheduledAt = new Date(`${new_date}T${new_time}:00`);
