...
const { blockedReason, BLOCK_MESSAGES } = require('../availability-blocks');
...
const tools = {
  ...
  schedule_appointment: async (params, user) => {
    try {
      ...
      // Lo que el doctor bloqueó en su disponibilidad tampoco se agenda por voz.
      const bloqueo = await blockedReason(user.id, `${date}T${time}`);
      if (bloqueo) return { success: false, error: BLOCK_MESSAGES[bloqueo] };
  ...
  register_patient: async (params, user) => {
    try {
      const { name, identity_number, phone } = params;
      if (!name || !identity_number || !phone) {
        return { success: false, error: 'Nombre, número de identidad y teléfono son requeridos' };
      }

      const result = await query(`
        INSERT INTO patients (clinic_id, name, identity_number, phone)
        VALUES ($1, $2, $3, $4)
        RETURNING id
      `, [user.clinic_id, name.trim(), String(identity_number).trim(), String(phone).trim()]);

      // Crear la fila de info crítica como hace el alta por UI, para que el
      // paciente quede consistente (mismas suposiciones en el resto de la app).
      await query(
