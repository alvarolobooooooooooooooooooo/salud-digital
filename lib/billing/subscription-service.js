...
const { getProvider, SUBSCRIPTION_STATUS, PaymentError, PROVEEDOR_CORTESIA } = require('../payments/provider');
...
async function buscarSuscripcion(clinicId, { excluirCortesia = false } = {}) {
  if (!clinicId) return null;
  const r = await query(
    `SELECT s.*, p.code AS plan_code, p.name AS plan_name, p.provider_refs AS plan_provider_refs, p.features AS plan_features
      ...
      ORDER BY (CASE WHEN s.status IN ('active','trialing','past_due') THEN 0 ELSE 1 END),
               ...
               s.id DESC
      LIMIT 1`,
    [clinicId, excluirCortesia ? PROVEEDOR_CORTESIA : ''],
  );
...
async function getForClinic(clinicId) {
  return buscarSuscripcion(clinicId);
}

/**
 * La suscripción DE PAGO de una clínica, saltándose la prueba de cortesía.
 * La usa el checkout: lo que se cobra es siempre una suscripción de pago, y la
 * regalada ni se cobra ni se convierte en una. Sin esto, cada intento de pago
 * durante una prueba creaba una suscripción nueva en el procesador, porque la
 * de cortesía tapaba a la que ya estaba a medias.
 */
async function getPayableForClinic(clinicId) {
  return buscarSuscripcion(clinicId, { excluirCortesia: true });
}
...
function access(sub, ahora = new Date()) {
  ...
  if (ESTADOS_CON_ACCESO.includes(sub.status)) {
    // Una prueba termina el día que dice, y ese día termina el acceso con
    // ella. Sin esta comprobación, un 'trialing' con la fecha ya pasada seguía
    // abriendo la plataforma indefinidamente: el estado solo cambia cuando
    // corre el job de facturación, y basta con que no corra una noche —o con
    // que la instancia se haya reiniciado— para regalar meses.
    if (sub.status === SUBSCRIPTION_STATUS.TRIALING && sub.current_period_end && !dentroDelPeriodo) {
...
async function start({ clinicId, userId, planCode, email, name, returnUrl, cancelUrl, providerName }) {
  ...
  // Una prueba de cortesía NO bloquea el alta. El caso normal es justo ese —el
  // doctor contrata antes de que se le acabe la prueba— y contarla como "ya
  // suscrito" dejaba sin forma de pagar precisamente a quien quería pagar. La
  // prueba se cierra sola cuando entra el primer cobro (markPaymentSucceeded).
  const existente = await getForClinic(clinicId);
  if (
    existente &&
    access(existente).active &&
    existente.status !== SUBSCRIPTION_STATUS.CANCELLED &&
    existente.provider !== PROVEEDOR_CORTESIA
  ) {
...
// ── Prueba de cortesía ──
//
// La concede a mano el administrador de la plataforma desde su panel (por
// ejemplo, al aprobar la solicitud de un doctor). No pasa por ningún
// procesador: es una fila de `subscriptions` con provider 'cortesia', estado
// 'trialing' y una fecha de fin. A partir de ahí el resto del sistema la trata
// como a cualquier otra suscripción —da acceso mientras dure y caduca sola—,
// que es justo lo que se quería: nada de un interruptor "regalado" aparte, que
// habría que recordar en cada sitio donde se comprueba el pago.

/**
 * Referencia de la fila de cortesía. La columna `provider_subscription_id` es
 * NOT NULL en la base desplegada (viene del `external_id` de la primera
 * iteración), así que hay que darle una: una por clínica, con el formato que
 * aceptan las rutas que reciben referencias del navegador
 * (/^[A-Za-z0-9_-]{6,64}$/), para que ninguna de ellas la rechace por su forma.
 */
function refCortesia(clinicId) {
  return `${PROVEEDOR_CORTESIA}-${clinicId}`;
}

/** La fila de cortesía de una clínica, si la tiene. Solo puede haber una. */
async function filaCortesia(clinicId) {
  const r = await query(
    `SELECT * FROM subscriptions
      WHERE clinic_id = $1 AND provider = $2
      ORDER BY COALESCE(current_period_end, created_at) DESC, id DESC
      LIMIT 1`,
    [clinicId, PROVEEDOR_CORTESIA],
  );
  return r.rows[0] || null;
}

/**
 * Concede (o extiende) un periodo de prueba gratuito a una clínica.
 *
 * @param {number} clinicId
 * @param {string} [planCode]  qué plan disfruta durante la prueba — de él salen
 *                             las funciones desbloqueadas y el importe que se
 *                             cobrará después.
 * @param {number} [months]    1 a 24.
 * @param {number} [userId]    el profesional para el que se concede (traza).
 * @param {number} [grantedBy] quién la concedió (el super_admin).
 * @returns {Promise<{subscription:object, plan:object, trialEndsAt:Date, extended:boolean}>}
 */
async function grantTrial({ clinicId, planCode, months = 6, userId = null, grantedBy = null, note = '' } = {}) {
  const clinica = parseInt(clinicId, 10);
  if (!Number.isInteger(clinica)) {
    throw new PaymentError('La prueba necesita una clínica', { code: 'no_clinic' });
  }

  const meses = parseInt(months, 10);
  if (!Number.isInteger(meses) || meses < 1 || meses > 24) {
    throw new PaymentError('La prueba tiene que ir de 1 a 24 meses', { code: 'invalid_trial_length' });
  }
  ...
  // Regalar meses a quien ya está pagando sería cobrarle y no darle nada a
  // cambio: getForClinic elegiría la prueba (termina más tarde) y su
  // suscripción de pago quedaría escondida detrás.
  const vigente = await getForClinic(clinica);
  if (vigente && vigente.provider !== PROVEEDOR_CORTESIA && access(vigente).active) {
    throw new PaymentError('Esta clínica ya tiene una suscripción vigente.', { code: 'already_subscribed' });
  }

  const ahora = new Date();
  const anterior = await filaCortesia(clinica);

  // Extender una prueba viva encadena desde donde termina, no desde hoy: si no,
  // "seis meses más" a mitad de la prueba serían menos de seis meses.
  const restante =
    anterior && anterior.current_period_end && new Date(anterior.current_period_end) > ahora
      ? new Date(anterior.current_period_end)
      : null;
  const fin = addInterval(restante || ahora, 'month', meses);

  const marca = JSON.stringify({
    cortesia: {
      concedida_en: ahora.toISOString(),
      meses,
      hasta: fin.toISOString(),
      por: grantedBy,
      nota: String(note || '').slice(0, 400),
    },
  });

  // Una clínica no acumula filas de cortesía: si ya tenía una, se reescribe.
  // Dos pruebas vivas a la vez harían que "hasta cuándo" dependiera de cuál
  // eligiera el ORDER BY.
  const r = anterior
    ? await query(
        `UPDATE subscriptions
            SET status = 'trialing',
                plan_id = $2, amount = $3, currency = $4,
                billing_interval = $5, interval_count = 1,
          ...
          WHERE id = $1
          RETURNING *`,
        [anterior.id, plan.id, plan.amount, plan.currency, plan.billing_interval, fin, userId || null, marca],
      )
    : await query(
        `INSERT INTO subscriptions
           (clinic_id, user_id, provider, provider_subscription_id, status, plan_id, amount, currency,
            billing_interval, interval_count, current_period_start, current_period_end,
            trial_ends_at, next_billing_at, metadata, updated_at)
         VALUES ($1,$2,$3,$4,'trialing',$5,$6,$7,$8,1,CURRENT_TIMESTAMP,$9,$9,$9,$10::jsonb,CURRENT_TIMESTAMP)
         RETURNING *`,
        [clinica, userId || null, PROVEEDOR_CORTESIA, refCortesia(clinica), plan.id, plan.amount,
          plan.currency, plan.billing_interval, fin, marca],
      );

  const suscripcion = normalizarSuscripcion(r.rows[0]);
  await espejarEnClinica(suscripcion);
  await auditar({
    userId: grantedBy,
    clinicId: clinica,
    action: 'billing.trial.granted',
    input: { meses, plan: plan.code, hasta: fin.toISOString(), extendida: !!restante, nota: note || '' },
  });

  return { subscription: suscripcion, plan, trialEndsAt: fin, extended: !!restante };
}

/**
 * El primer cobro real cierra la prueba de cortesía de esa clínica. Dejarla
 * viva significaba que `getForClinic` seguiría eligiéndola —termina más tarde—
 * y la pantalla del doctor diría "en prueba" mientras se le está cobrando.
 */
async function cerrarCortesia(clinicId) {
  if (!clinicId) return null;
  const r = await query(
    `UPDATE subscriptions
        SET status = 'expired',
            cancel_reason = 'Sustituida por una suscripción de pago',
            next_billing_at = NULL,
            updated_at = CURRENT_TIMESTAMP
      WHERE clinic_id = $1 AND provider = $2 AND status IN ('trialing','incomplete')
      RETURNING id, current_period_end`,
    [clinicId, PROVEEDOR_CORTESIA],
  );
  const fechas = r.rows
    .map((f) => f.current_period_end)
    .filter(Boolean)
...
async function syncFromProvider(sub) {
  ...
  // Un procesador que no sabe releer no puede corregir nada, y no es un error:
  // lo que hay en la base es todo lo que hay. Es el caso de una prueba de
  // cortesía, que no vive en ningún procesador — sin esto, el botón
  // "Actualizar estado" de la pantalla del doctor devolvía un 501 durante toda
  // la prueba.
  let remota;
  try {
    remota = await provider.getSubscription(sub.provider_subscription_id);
  } catch (err) {
    if (err.code === 'unsupported_operation') return sub;
    throw err;
  }
...
async function markPaymentSucceeded(sub, { amount, currency, paidAt = new Date(), providerPaymentId, periodEnd } = {}) {
  ...
  // La prueba de cortesía se cierra con el primer cobro, pero NO se tira: el
  // mes pagado empieza donde ella terminaba. Cobrar desde hoy sería quedarnos
  // con los días que ya habíamos regalado, y castigaría justo a quien se
  // adelanta a pagar.
  const finPrueba = sub.provider !== PROVEEDOR_CORTESIA ? await cerrarCortesia(sub.clinic_id) : null;
  ...
  let base =
    sub.current_period_end && new Date(sub.current_period_end).getTime() > paidAt.getTime()
      ...
      : paidAt;
  if (finPrueba && finPrueba.getTime() > base.getTime()) base = finPrueba;
 ...
 * La prueba de cortesía tampoco tiene margen: se regaló con una fecha y esa
 * fecha es toda la promesa que hay.
 */
    ...
    `UPDATE subscriptions
      ...
      WHERE current_period_end IS NOT NULL
        AND (
           ...
           OR (status = 'trialing' AND current_period_end < $1)
