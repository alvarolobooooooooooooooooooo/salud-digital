...
async function sendDoctorInvitation({ to, doctorName, clinicName, token }) {
  const acceptUrl = `${process.env.APP_URL}/accept-invitation.html?token=${token}`;
  try {
    await sgMail.send({
      to,
      from: process.env.EMAIL_FROM,
      subject: `Invitación para unirte a ${clinicName} — Salud Digital`,
      ...
      html: `
        <div style="font-family:sans-serif;max-width:520px;margin:auto;padding:32px;border:1px solid #e5e7eb;border-radius:12px">
          ...
          <p style="color:#374151">Has sido invitado a unirte a <b>${clinicName}</b> como médico en la plataforma <b>Salud Digital</b>, donde podrás gestionar tus pacientes y expedientes clínicos de forma segura.</p>
          <p style="color:#374151">Haz clic en el botón para crear tu cuenta y acceder:</p>
          <a href="${acceptUrl}" style="display:inline-block;margin:24px 0;background:#2563eb;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:600">
            Aceptar invitación
          </a>
