 ...
 * marcó en su calendario de disponibilidad:
 *   'closed'  → cerró el día entero
 *   'blocked' → quitó esa hora
...
async function blockedReason(doctorId, scheduledAt) {
  ...
  const blocked = Array.isArray(override.blocked_times) ? override.blocked_times : [];
  if (blocked.length === 0) return null;

  const durationRow = await query(
    'SELECT slot_duration FROM doctor_availability WHERE doctor_id = $1 LIMIT 1',
    [doctorId]
  );
  const duration = Number(durationRow.rows[0] && durationRow.rows[0].slot_duration) || DEFAULT_SLOT_MINUTES;
