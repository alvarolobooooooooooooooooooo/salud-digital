...
async function createSubscription({
  ...
  // El plan lo manda quien llama (sale de la tabla `plans`); PAYPAL_PLAN_ID solo
  // se usa como respaldo para instalaciones antiguas sin catálogo en la BD.
  const plan = String(planExplicito || planId() || '').trim();
  if (!plan) {
    throw new Error('Falta el plan de PayPal. Ejecuta: node tools/paypal-setup.js');
  }
  ...
  const body = {
    ...
    application_context: {
      ...
      // es-HN, no es-ES: el locale decide el país y el prefijo telefónico que
      // PayPal precarga en el formulario de tarjeta. Con 'es-ES' el comprador
      // hondureño se encontraba "España" y "+34" y tenía que corregirlos a mano.
      // (Acepta 'es-HN' con guion; con guion bajo lo rechaza.)
      locale: 'es-HN',
...
// ── Órdenes (pago único) ──
//
// Es la vía que SÍ permite el formulario de tarjeta embebido en nuestra página
// (comprobado: con intent=capture el botón de tarjeta despliega los campos in
// situ; con intent=subscription exige la ventana de PayPal). A cambio, cada
// cobro lo inicia el cliente: no se puede cobrar sin que esté delante.

async function createOrder({ amount, currency, description, customId, requestId, brandName }) {
  const body = {
    intent: 'CAPTURE',
    purchase_units: [
      {
        amount: { currency_code: currency, value: Number(amount).toFixed(2) },
        description: String(description || 'Salud Digital').slice(0, 127),
        custom_id: String(customId || '').slice(0, 127),
      },
    ],
    application_context: {
      brand_name: brandName || process.env.SUBSCRIPTION_BRAND_NAME || 'Salud Digital',
      locale: 'es-HN',
      shipping_preference: 'NO_SHIPPING',
      user_action: 'PAY_NOW',
    },
  };
  return papi('POST', '/v2/checkout/orders', body, requestId ? { 'PayPal-Request-Id': String(requestId) } : {});
}

// Idempotente por PayPal-Request-Id: si la respuesta se pierde por red, repetir
// la captura con la misma clave no cobra dos veces.
...
async function getOrder(orderId) {
  return papi('GET', `/v2/checkout/orders/${encodeURIComponent(orderId)}`);
}

// Cambio de plan de una suscripción viva. PayPal devuelve un enlace de
// aprobación cuando el importe nuevo exige que el pagador vuelva a autorizar.
async function reviseSubscription(id, newPlanId) {
  return papi('POST', `/v1/billing/subscriptions/${encodeURIComponent(id)}/revise`, {
    plan_id: newPlanId,
  });
}

// Detalle de un cobro concreto (los ids que llegan en PAYMENT.SALE.*).
async function getSale(saleId) {
  return papi('GET', `/v1/payments/sale/${encodeURIComponent(saleId)}`);
}
...
module.exports = {
  env,
  // El client id es público por diseño: va en la URL del SDK que carga el
  // navegador. El secreto NUNCA sale del servidor.
  clientId,
  isConfigured,
  hasPlan,
  planId,
