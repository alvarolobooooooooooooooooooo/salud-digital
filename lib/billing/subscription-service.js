...
function access(sub, ahora = new Date()) {
  ...
  if (ESTADOS_CON_ACCESO.includes(sub.status)) {
    ...
    return dentroDelPeriodo
      ? { active: true, reason: 'past_due_grace' }
...
async function markPaymentSucceeded(sub, { amount, currency, paidAt = new Date(), providerPaymentId, periodEnd } = {}) {
  ...
  // Si el procesador no dice hasta cuándo, se calcula desde el cobro.
  const periodo = periodEnd
    ? { start: paidAt, end: new Date(periodEnd) }
    : periodFrom(paidAt, intervalo, cuenta);
...
async function expireOverdue(ahora = new Date()) {
  const r = await query(
    `UPDATE subscriptions
      ...
      WHERE status IN ('cancelled','payment_failed','past_due')
