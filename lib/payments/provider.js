...
const REGISTRO = {
  ...
  // No cobra nada: es el titular de las pruebas que concede el administrador
  // desde el panel. Está en el registro para que `getProvider` nunca reviente
  // con una suscripción de cortesía delante. Ver su cabecera.
  cortesia: () => require('./providers/cortesia'),
...
// El titular de las pruebas de cortesía. Vive en el registro para que
// `getProvider` lo encuentre, pero NUNCA es una forma de pago: se filtra de la
// lista de procesadores ofrecidos y del procesador por defecto, o una errata en
// el entorno (PAYMENTS_PROVIDER=cortesia) dejaría la plataforma entera gratis.
const PROVEEDOR_CORTESIA = 'cortesia';

function nombreProviderPorDefecto() {
  // Se tolera que venga una lista: confundir PAYMENTS_PROVIDER con
  // PAYMENTS_PROVIDERS es el error natural, y castigarlo dejaba la plataforma
  // sin NINGÚN procesador reconocido —sin poder cobrar y sin decir por qué—.
  // Con una lista manda el primero.
  const raw = String(process.env.PAYMENTS_PROVIDER || 'paypal');
  const elegido = raw.split(',')[0].trim().toLowerCase();
  return !elegido || elegido === PROVEEDOR_CORTESIA ? 'paypal' : elegido;
}

/**
 * Procesadores ofrecidos al cliente, en orden de preferencia. Existe porque un
 * mismo procesador puede no cubrir a todo el mundo: PayPal cobra solo cada mes,
 * pero para eso exige que el comprador tenga cuenta PayPal; su pago único acepta
 * tarjeta de cualquiera, pero entonces cada mes lo paga el cliente a mano. Con
 * los dos activos, cada doctor elige el que puede usar.
 *
 * PAYMENTS_PROVIDERS=paypal,paypal_onetime  ← lo que ve el cliente
 * PAYMENTS_PROVIDER=paypal                  ← el de siempre, sigue siendo el
 *                                             predeterminado si no se pide otro
 */
function enabledProviderNames() {
  // Si no hay lista propia, sirve la de PAYMENTS_PROVIDER — que normalmente
  // trae uno solo, pero si alguien escribió ahí los dos, se respetan igual.
  const raw = String(process.env.PAYMENTS_PROVIDERS || process.env.PAYMENTS_PROVIDER || 'paypal').trim();
  const lista = raw.split(',').map((s) => s.trim().toLowerCase()).filter(Boolean);
  return [...new Set(lista)].filter((n) => REGISTRO[n] && n !== PROVEEDOR_CORTESIA);
}
