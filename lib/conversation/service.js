...
class ConversationService {
  ...
  // Verifica que la sesión pertenezca a quien la pide. Sin esto, cualquier
  // usuario autenticado podría leer/continuar el chat (con PHI) de otra clínica
  // pasando un sessionId ajeno. Devuelve true solo si user_id y clinic_id coinciden.
  async ownsSession(sessionId, userId, clinicId) {
    const r = await query(
      'SELECT 1 FROM conversation_sessions WHERE id = $1 AND user_id = $2 AND clinic_id = $3',
      [sessionId, userId, clinicId]
    );
    return r.rowCount > 0;
  }
